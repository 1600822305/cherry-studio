...
const TTSSettings: FC = () => {
  ...
  const handleAddVoice = () => {
    ...
    dispatch(addTtsCustomVoice(newVoice))
    setNewVoice('')
  }
  ...
  const handleAddModel = () => {
    ...
    dispatch(addTtsCustomModel(newModel))
    setNewModel('')
  }

  // 删除自定义音色
  const handleRemoveVoice = (voice: string) => {
    dispatch(removeTtsCustomVoice(voice))
  }

  // 删除自定义模型
  const handleRemoveModel = (model: string) => {
    dispatch(removeTtsCustomModel(model))
  }
            ...
            <>
              ...
              <Form.Item label={t('settings.tts.voice')} style={{ marginBottom: 8 }}>
                <Select
                  ...
                  options={ttsCustomVoices.map((voice: string) => ({ label: voice, value: voice }))}
                  disabled={!ttsEnabled}
                  style={{ width: '100%' }}
              ...
              <TagsContainer>
                {ttsCustomVoices && ttsCustomVoices.length > 0 ? (
                  ttsCustomVoices.map((voice: string) => (
                    <Tag
                      key={voice}
                      closable
                      onClose={() => handleRemoveVoice(voice)}
              ...
              <Form.Item label={t('settings.tts.model')} style={{ marginBottom: 8, marginTop: 16 }}>
                <Select
                  ...
                  options={ttsCustomModels.map((model: string) => ({ label: model, value: model }))}
                  disabled={!ttsEnabled}
                  style={{ width: '100%' }}
              ...
              <TagsContainer>
                {ttsCustomModels && ttsCustomModels.length > 0 ? (
                  ttsCustomModels.map((model: string) => (
                    <Tag
                      key={model}
                      closable
